...
class _BookingServiceStep1State extends State<BookingServiceStep1> {
  ...
  void init() async {
    ...
    if (isUpdate) {
      ...
      if (!tempSlot.slot.validate().contains(widget.bookingData!.booking_slot.validate())) {
        slotsList
            .firstWhere((element) => element.day.validate().toLowerCase() == selectedHorizontalDate.weekday.weekDayName.validate().toLowerCase())
            .slot!
            .add(widget.bookingData!.booking_slot.validate());
      }
    }
  }
  ...
  void _handleNextButtonClick() {
    if (isUpdate) {
      if (widget.data!.serviceDetail!.bookingSlot.validate() == widget.bookingData!.booking_slot.validate()) return toast(language.pleaseSelectDifferentSlotThenPrevious);
  ...
  void updateDetails() async {
    ...
    await updateBooking(request).then((value) {
      ...
      widget.bookingData!.booking_slot = widget.data!.serviceDetail!.bookingSlot;
                  ...
                  temp.isNotEmpty
                      ? AvailableSlotsComponent(
                          ...
                          selectedSlots: widget.data!.serviceDetail!.bookingSlot.validate().isNotEmpty
                              ...
                              : isUpdate
                                  ? [widget.bookingData!.booking_slot.validate()]
                                  : [],
                          isProvider: false,
                          availableSlots: temp,
                          selectedDate: selectedHorizontalDate,
